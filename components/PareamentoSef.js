...
export default function PareamentoSef() {
    ...
    const timer = setInterval(() => {
      setProgress(prev => {
        ...
        if (newProgress >= 1) {
          clearInterval(timer);
          setIsComplete(true);
          setTimeout(() => navigation.replace('SefPareado'), 500);
          return 1;
        }
        return newProgress;
      });
  ...
  return (
    <View style={styles.container}>
      <Image
        source={require('../assets/Pareamento.png')}
        style={styles.img}
      />
      <Text style={styles.pareando}>
        {isComplete ? "Dispositivo pareado" : "Pareando SEF 1..."}
      </Text>
      <ProgressBar 
        progress={progress} 
        color="#2e9bff" 
